    ...
    <div className="fixed bottom-6 right-6 z-50">
      {!isOpen ? (
        <button
          ...
          className="btn-primary py-3.5 px-5 rounded-full shadow-2xl flex items-center gap-2 font-bold group animate-bounce cursor-pointer"
        >
          <div className="w-7 h-7 rounded-full bg-white text-emerald-600 flex items-center justify-center">
            <Sparkles className="w-4 h-4" />
          </div>
          <span className="text-xs text-white">Ask FlashBot AI</span>
        </button>
      ) : (
        <div className="w-80 sm:w-96 glass-panel border-emerald-500/40 shadow-2xl rounded-2xl overflow-hidden flex flex-col h-[500px]">
          ...
          <div className="p-4 bg-gradient-to-r from-slate-950 via-slate-900 to-slate-950 border-b border-white/10 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <h4 className="text-xs font-bold text-white font-['Plus_Jakarta_Sans']">FlashBot AI US Assistant</h4>
                <span className="text-[10px] text-emerald-400 font-semibold flex items-center gap-1">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping"></span> Live US Consultation
                </span>
              </div>
            </div>
                  ...
                  <div className="mt-2 space-y-1 w-full">
                    {msg.suggestions.map((sug, i) => (
                      <button
                        ...
                        className="w-full text-left p-2 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-300 text-[11px] font-medium transition-colors border-none cursor-pointer"
                      >
                        ⚡ {sug}
                      </button>
                    ))}
          ...
          >
            <input
              ...
              className="flex-1 bg-white/5 border border-white/15 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-emerald-400"
            />
            <button
              type="submit"
              className="w-8 h-8 rounded-xl bg-emerald-600 text-white flex items-center justify-center hover:scale-105 transition-transform border-none cursor-pointer"
            >
